...
class Couchdb_DApp:
    ...
    def startup(dapp_handler):
        ...
        for index, _ in enumerate(client_config['priv_ips']):
            ...
            stdin, stdout, stderr = ssh_clients[index].exec_command(f"{couchdb_config['join_command']}")
            out = stdout
            ...
            stdin, stdout, stderr = ssh_clients[index].exec_command(f"curl http://{couchdb_config['priv_ips'][0]}:5984")
            wait_and_log(stdout, stderr)

        stdin, stdout, stderr = ssh_clients[0].exec_command((f"curl -X PUT http://{couchdb_config['priv_ips'][0]}:5984/test"))
        wait_and_log(stdout, stderr)
        ...
        for index, _ in enumerate(couchdb_config['priv_ips']):
            Couchdb_DApp.write_config(client_config, couchdb_config['priv_ips'][index])
            scp_clients[index].put(f"{client_config['exp_dir']}/setup", "/home/ubuntu", recursive=True)
    ...
    def write_config(client_config, ip):
        ...
        config["couch_address"] = f"http://admin:password@{ip}:5984"
